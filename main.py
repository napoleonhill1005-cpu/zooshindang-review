...
환경변수:
  ...
  GOOGLE_ACCOUNT_ID / GOOGLE_LOCATION_ID  구글 비즈니스 프로필 ID
  NAVER_COOKIE / CATCHTABLE_TOKEN  실제 수집 시 인증값
  GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET / GOOGLE_REFRESH_TOKEN  구글 OAuth
"""
import base64
import json
import os
...
def collect():
    ...
    if not _USE_MOCK:
        ...
        if os.environ.get("GOOGLE_REFRESH_TOKEN") and len(g_reviews) == 0:
            _send_slack_alert(
                "⚠️ *구글 리뷰 수집 0건*\n"
                "GOOGLE_REFRESH_TOKEN 무효 또는 API 권한 문제 의심. "
